...
| `demo_prior_week_1`, `demo_prior_week_2` | *(not edge cases)* the two weeks **before** `demo_week`, for demo mode's own-baseline comparison | The result screen compares this week to the same person's earlier weeks. On a real phone those are the cached vectors of previous runs, so a fresh install — and therefore every demo — has none, and every row would show a value with no direction. Same person and same nights as `demo_week`; what differs is a long drifting afternoon away-block and a busier comms week, so the demo reads screen ↑, rest →, calls ↓, rhythm → |
