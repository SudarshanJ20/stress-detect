...
| **gps** | `gps_u##.csv` | `time, provider, network_type, accuracy, latitude, longitude, altitude, bearing, speed, travelstate` | ~20 min | 49/49 | Periodic fixes. `travelstate` ∈ {stationary, moving}. Sparse rows (u00 ≈ 4.3k). ⚠️ **Data-quality bug:** several `gps` files contain the header row **repeated mid-file** (concatenated Kaggle export — e.g. `gps_u01` line 4318). Any parser must coerce `time`/`latitude`/`longitude` to numeric and **drop non-numeric rows**, or `to_datetime(unit="s")` fails on the string `time`/`network`. Other sensing streams checked did not show this. |
